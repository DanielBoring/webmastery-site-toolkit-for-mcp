...
**2. Install Webmastery Site Toolkit for MCP Plugin**
Download the [latest release zip](https://github.com/DanielBoring/webmastery-site-toolkit-for-mcp/releases/latest) → WP Admin → Plugins → Add New → Upload Plugin → Install & Activate
...
MCP Adapter plugin                        ← framework: registers the REST endpoint,
  (WordPress/mcp-adapter)                     handles the MCP session, routes calls
        ...
        ▼
Webmastery Site Toolkit for MCP plugin               ← this plugin: registers the actual
...
New abilities and feature requests are tracked in [GitHub Issues](https://github.com/DanielBoring/webmastery-site-toolkit-for-mcp/issues).
...
| --------------------- | ---------------------------------------------------------------------- | ------------------- | --------- |
| `webmastery-site-toolkit-for-mcp/list-posts`   | List posts with filters (status, search, author, category, pagination) | `edit_posts`        | Author †  |
| `webmastery-site-toolkit-for-mcp/get-post`     | Get a single post by ID                                                | `edit_post`         | Author †  |
| `webmastery-site-toolkit-for-mcp/create-post`  | Create a new post with title, content, status, categories, tags        | `edit_posts`        | Author    |
| `webmastery-site-toolkit-for-mcp/update-post`  | Update an existing post                                                | `edit_post`         | Author †  |
| `webmastery-site-toolkit-for-mcp/delete-post`  | Move a post to trash                                                   | `delete_post`       | Author †  |
| `webmastery-site-toolkit-for-mcp/restore-post` | Restore a post from trash                                              | `delete_post`       | Author †  |
...
| Ability               | Description               | Required Capability | Min. Role |
| --------------------- | ------------------------- | ------------------- | --------- |
| `webmastery-site-toolkit-for-mcp/list-pages`   | List pages with filters   | `edit_pages`        | Editor    |
| `webmastery-site-toolkit-for-mcp/get-page`     | Get a single page by ID   | `edit_post`         | Editor    |
| `webmastery-site-toolkit-for-mcp/create-page`  | Create a new page         | `edit_pages`        | Editor    |
| `webmastery-site-toolkit-for-mcp/update-page`  | Update an existing page   | `edit_post`         | Editor    |
| `webmastery-site-toolkit-for-mcp/delete-page`  | Move a page to trash      | `delete_post`       | Editor    |
| `webmastery-site-toolkit-for-mcp/restore-page` | Restore a page from trash | `delete_post`       | Editor    |

### Taxonomy
| Ability                  | Description                         | Required Capability | Min. Role  |
| ------------------------ | ----------------------------------- | ------------------- | ---------- |
| `webmastery-site-toolkit-for-mcp/list-categories` | List all categories                 | `read`              | Subscriber |
| `webmastery-site-toolkit-for-mcp/list-tags`       | List all tags                       | `read`              | Subscriber |
| `webmastery-site-toolkit-for-mcp/create-category` | Create a new category               | `manage_categories` | Editor     |
| `webmastery-site-toolkit-for-mcp/create-tag`      | Create a new tag                    | `manage_categories` | Editor     |
| `webmastery-site-toolkit-for-mcp/delete-category` | Permanently delete a category by ID | `manage_categories` | Editor     |
| `webmastery-site-toolkit-for-mcp/delete-tag`      | Permanently delete a tag by ID      | `manage_categories` | Editor     |

### Comments
| Ability                  | Description                                       | Required Capability | Min. Role |
| ------------------------ | ------------------------------------------------- | ------------------- | --------- |
| `webmastery-site-toolkit-for-mcp/list-comments`   | List comments with filters (post, status, search) | `moderate_comments` | Editor    |
| `webmastery-site-toolkit-for-mcp/approve-comment` | Approve a comment                                 | `moderate_comments` | Editor    |
| `webmastery-site-toolkit-for-mcp/trash-comment`   | Move a comment to trash                           | `moderate_comments` | Editor    |
| `webmastery-site-toolkit-for-mcp/spam-comment`    | Mark a comment as spam                            | `moderate_comments` | Editor    |

### Media
| Ability                | Description                                             | Required Capability | Min. Role |
| ---------------------- | ------------------------------------------------------- | ------------------- | --------- |
| `webmastery-site-toolkit-for-mcp/list-media`    | List media items with filters (MIME type, search, pagination) | `upload_files`      | Author    |
| `webmastery-site-toolkit-for-mcp/get-media`     | Get a single media item by ID                           | `edit_post`         | Author †  |
| `webmastery-site-toolkit-for-mcp/update-media`  | Update media alt text, title, and caption               | `edit_post`         | Author †  |
| `webmastery-site-toolkit-for-mcp/delete-media`  | Permanently delete a media item                         | `delete_post`       | Author †  |
...
| Ability              | Description                                             | Required Capability | Min. Role     |
| -------------------- | ------------------------------------------------------- | ------------------- | ------------- |
| `webmastery-site-toolkit-for-mcp/list-users`  | List users with filters (role, search, pagination)      | `list_users`        | Administrator |
| `webmastery-site-toolkit-for-mcp/get-user`    | Get a single user by ID                                 | `list_users`        | Administrator |

### Plugins
| Ability                    | Description                                                                                     | Required Capability | Min. Role     |
| -------------------------- | ----------------------------------------------------------------------------------------------- | ------------------- | ------------- |
| `webmastery-site-toolkit-for-mcp/list-plugins`      | List installed plugins with version, activation state, network activation, auto-update, and update availability | `activate_plugins`  | Administrator |
| `webmastery-site-toolkit-for-mcp/activate-plugin`   | Activate a plugin by canonical plugin basename, or by slug when it resolves to exactly one plugin | `activate_plugins`  | Administrator |
| `webmastery-site-toolkit-for-mcp/deactivate-plugin` | Deactivate a plugin by canonical plugin basename, with protected-plugin and multisite safeguards | `activate_plugins`  | Administrator |
...
| Ability                    | Description                                                                                                | Required Capability | Min. Role  |
| -------------------------- | ---------------------------------------------------------------------------------------------------------- | ------------------- | ---------- |
| `webmastery-site-toolkit-for-mcp/site-health-check` | Run WordPress's built-in health tests; returns results grouped by severity (critical / recommended / good) | `manage_options`    | Administrator |

### Security Audit
| Ability                 | Description                                                                                                                               | Required Capability | Min. Role  |
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ------------------- | ---------- |
| `webmastery-site-toolkit-for-mcp/security-audit` | Check for common security issues: debug mode, file editor, SSL, admin username, WP/plugin version currency, XMLRPC, and auth key strength | `manage_options`    | Administrator |
...
| Ability                    | Description                                                                                                                                     | Required Capability | Min. Role  |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- | ---------- |
| `webmastery-site-toolkit-for-mcp/seo-analyze-post`  | Analyze a single post or page: title length, word count, meta description, focus keyword placement, image alt text, internal links, slug length | `edit_post`         | Author †   |
| `webmastery-site-toolkit-for-mcp/seo-site-overview` | Site-wide SEO snapshot: sitemap and robots.txt accessibility, count of published posts missing Yoast focus keyword or meta description          | `manage_options`    | Administrator |
...
> **Coming soon to the WordPress Plugin Directory** — this plugin has been submitted for review. Once approved, you'll be able to install it directly from **WP Admin → Plugins → Add New** by searching "Webmastery Site Toolkit for MCP". Until then, use one of the options below.
...
3. Upload `webmastery-site-toolkit-for-mcp.zip` and click **Install Now**
...
- `webmastery-site-toolkit-for-mcp/security-audit` with an Administrator service account — *"Run a security audit of my WordPress site"*
- `webmastery-site-toolkit-for-mcp/site-health-check` with an Administrator service account — *"Check WordPress site health"*
- `webmastery-site-toolkit-for-mcp/list-plugins` with an Administrator service account — *"List installed plugins and their activation state"*
