...
## Post metadata authorization regressions

`post-meta-authorization-fixture.php` supplies the `wstm110_*` metadata cases. It registers a callback requiring an actual draft post, stored gate metadata, and `manage_options` or the fixture's `wstm110_manage_meta` capability. The callback throws if invoked against a fabricated ID. Cases cover explicit reads, filtered listings, first writes, no-ops, deletion-specific capabilities, mixed metadata batches, generic and SEO-alias post/page inputs, global/subtype registrations, unknown/protected keys, unrestricted defaults, and final capability denials.

Every case using `setup.wstm110_meta_auth` or `setup.wstm110_state` snapshots the full post fields, stored metadata, and post/page ID set. Expected failures must leave those unchanged, including create failures. Successful creates must add exactly the returned post/page ID with the requested title, content, and status; positive batch writes are checked against stored metadata in addition to the existing `assert_post_meta` assertions. Registered protected SEO keys with explicit `__return_true` callbacks and a restrictive global key overridden by an unrestricted subtype policy have separate create-success cases with complete metadata assertions; the same global key without the subtype override must reject creation without partial persistence.

The restrictive-key fixture temporarily replaces only its key registrations and removes Yoast's `allow_custom_field_edits` primitive-capability grant, then restores them in the runner's `finally` block. Separate cases preserve the real Yoast grant to verify final WordPress capability semantics. Provider-backed cases keep the real plugins active and use their actual callbacks. The three original provider create-write cases now assert pre-insertion denial and retain all their original metadata assertions in explicit create-without-metadata, then authorized-update flows. The security validator pins those three real-provider denial labels and both page-provider denial labels to `setup.wstm110_state`, their pre-insertion error reason, and the absence of synthetic authorization replacement. Fixtures also model older/unregistered SEO keys to keep that compatibility path covered.

This create behavior is **breaking and requires major-release review**. Yoast 28.5's `inc/class-wpseo-meta.php` callback and SEOPress 10.2's `src/Actions/Api/Metas/AdvancedSettings.php::meta_auth()` both require `edit_post` against the supplied ID. Core 6.9 and the tested 7.1 runtime resolve metadata capabilities through the actual object's subtype; ID 0, synthetic objects, and insert/delete preflights are not valid substitutes. Creation checks create/publish capabilities and conservatively classifies key authorization hooks, not real-ID metadata capabilities. Object-dependent metadata `map_meta_cap`/`user_has_cap` evaluation occurs on the subsequent update; the global filters still run for the create/publish capability checks.

Run `composer qa` and both disposable Docker layers. The baseline coverage inventory above is supplemented by these regression cases; use `e2e-summary.json` for exact current counts. Note that the PHP runners currently write summaries under the checkout's `e2e-artifacts` directory even when the shell's `E2E_ARTIFACTS_DIR` is overridden.
