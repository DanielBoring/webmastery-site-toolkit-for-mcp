...
final class GithubSafeguardsTest extends TestCase {
	...
	public function test_normal_pr_ci_executes_static_release_and_compatibility_regressions(): void {
		...
		$composer = json_decode(file_get_contents($root . '/composer.json'), true, 512, JSON_THROW_ON_ERROR);
		self::assertSame(
			array('@test:phpstan-baseline', '@test:release-safeguards', 'bash tests/destructive-stage-test.sh', 'bash tests/input-schema-stage-test.sh', 'bash tests/compatibility-download-test.sh', 'bash tests/compatibility-dependency-policy-test.sh'),
