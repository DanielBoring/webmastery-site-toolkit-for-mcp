...
- [ ] Destructive runtime wiring retains eight serial boundary/trash modes, native audit before test MU capabilities, actual CLI/HTTP config parity, exclusive evidence before credentials, exact owned restoration and original failure status; mocked stages do not replace source/floor/original-package proof
...
- [ ] Untrusted runtime changes retain the complete source/package-bound case plan, original JSON types, exclusive evidence before credentials, no-config-write CLI/HTTP restoration, owned resource cleanup and fail-closed outer retention; mock-stage tests and predecessor CI do not establish dedicated runtime proof
- [ ] Untrusted evidence captures original wire/stdout/stderr privately before parsing, publishes only validated projections/safe witnesses, keeps host authority outside project binds, validates exact prepared generation/inventory and process outcomes, and retains failed/partial evidence after resource cleanup; synthetic, real POSIX capture/mocked Docker, native-Windows refusal/BLOCKED paths and genuine Ubuntu runtime results are reported separately
- [ ] The independent companion covers primary-clear capture/exit failures; all required evidence and authorization writes precede terminal companion unlink (release commit), and the public precommit-only receipt never implies committed release, a retained postcommit guard, or successful QA after failed/lost acknowledgment
